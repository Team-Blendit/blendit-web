...
export function NetworkingManageClient({
  networkingDetail,
  applicants,
  participants,
  comments,
}: NetworkingManageClientProps) {
    ...
    <>
      ...
      <div className="flex items-center justify-between self-stretch">
        <div className="flex items-center gap-[24px] flex-1">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <Badge color='red' style='solid' text={networkingDetail.status} />
          </div>
        </div>
        <button
          className="p-[8px]"
      ...
      <div className="flex items-start gap-[60px] self-stretch">
        ...
        <div className="w-[440px] shrink-0">
          <Card
            variant="postInfo"
            userName={networkingDetail.userName}
            userJob={networkingDetail.job}
            postDate={networkingDetail.userDate}
            meetDate={networkingDetail.date}
            meetLocation={networkingDetail.location}
            keywords={networkingDetail.keywords}
            currentNum={networkingDetail.currentMembers}
            totalNum={networkingDetail.totalMembers}
            openChatLink={networkingDetail.chatLink}
            buttonIcon={<GearIcon/>}
            buttonText='블렌딩 수정하기'
            onButtonClick={() => console.log('커피챗 신청하기')}
          />
        </div>
        ...
        <div className="flex-1 flex flex-col gap-[50px]">
          ...
          <PostDescription
            title="소개"
            content={networkingDetail.description}
          />
            ...
            >
              {applicants.map((applicant) => (
                <Card
                  ...
                  userJob={applicant.job}
                  userCareer={applicant.experience}
                  userLocation={applicant.region}
                  keywords={applicant.keywords}
                  showButton={true}
                  className="w-[273px] shrink-0"
                  onBookmarkClick={() => console.log('Bookmark clicked:', applicant.name)}
                />
              ))}
            </div>
          </div>
