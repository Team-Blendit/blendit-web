...
export const Card: React.FC<CardProps> = ({
  ...
  return (
    <div
      className={cn(
        'bg-(--bg-canvas) w-[342px] border border-(--border-default) rounded-[20px] p-[24px] flex flex-col gap-[16px] items-start',
        className
      )}
    >
      {/* Content */}
      <div className="flex flex-col gap-[16px] items-start self-stretch">
        ...
        <div className="flex flex-col gap-[8px] self-stretch">
          ...
          {/* Title */}
          <h3 className="text-[22px] font-normal text-(--text-primary) truncate">
            {title}
          </h3>
        </div>

        {/* Bottom Section */}
        <div className="flex flex-col gap-[16px] self-stretch items-start">
          {/* User Info */}
          <div className="flex gap-[16px] items-center self-stretch">
            {/* Profile Image */}
            <svg width="88" height="88" viewBox="0 0 88 88" fill="none" xmlns="http://www.w3.org/2000/svg">
              <rect width="88" height="88" rx="44" fill="#DBDBDB"/>
              <circle cx="42.5" cy="31.5" r="12.5" fill="white"/>
              <ellipse cx="44" cy="65.5" rx="26" ry="16.5" fill="white"/>
            </svg>
            ...
            <div className="flex-1 flex flex-col gap-[6px] items-start">
              ...
              <div className="flex gap-[8px] items-center self-stretch">
                <p className="text-[18px] font-normal text-(--text-primary) truncate max-w-[110px]">
                  {userName}
                </p>
                <Badge color="blue" style="outline" text={job} />
              </div>

              {/* Career and Location */}
              <div className="flex flex-col items-start gap-[4px] self-stretch">
                <p className="text-[18px] font-normal text-(--text-tertiary)">
                  {career}
                </p>
